class Container {
  ...
  capturePageSnapshot() {
    ...
    // Patch cloned document: replace oklch/color-mix with rgb fallbacks
    // because html2canvas can't parse modern CSS color functions
    function patchClonedDoc(doc) {
      const sheets = doc.querySelectorAll('style, link[rel="stylesheet"]')
      sheets.forEach(el => {
        if (el.tagName === 'STYLE') {
          let txt = el.textContent
          txt = txt.replace(/oklch\([^)]+\)/g, 'rgba(255,255,255,0.5)')
          txt = txt.replace(/color-mix\([^)]+\)/g, 'rgba(255,255,255,0.5)')
          el.textContent = txt
        }
      })
      // Also patch inline styles
      doc.querySelectorAll('[style]').forEach(el => {
        if (el.getAttribute('style').includes('oklch') || el.getAttribute('style').includes('color-mix')) {
